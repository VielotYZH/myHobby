...
Task* creatByHand() {
    ...
    head->next = NULL;
    Task* p = head;
    int userFloor, destination;
    printf("Please enter the calling floor and destination floor respectively\n");
    printf("The two are separated by a space,both values of 0 indicate the end\n");
    scanf("%d %d", &userFloor, &destination);
    Task* q = NULL;
    while (userFloor != 0) {
...
Task* creatByFile(FILE* fp,int* initialFloor) {
    ...
    int userFloor, destination;
    while (!feof(fp)) {
        fscanf(fp, "%d %d", &userFloor, &destination);
        q = (Task*)malloc(TASK_SIZE);
        q->userFloor = userFloor;
        q->destination = destination;
        q->next = NULL;
...
void scheduleElevator(Task* head, ElevatorState* elevatorState) {
    int ifEmpty = 1;
    int runTime = 0;
    while (!(head->next == NULL && ifEmpty == 1)) {
        //The task list is not empty or the elevator is not empty
